...
class SampleFunctionAndDerivative():
    ...
    def get_function_value(self, position):
        ...
        hull_value = (
            ...
            - relative_position.T.dot(self.matrix).dot(relative_position))
        return hull_value

    def evaluate_gradient(self, position):
        relative_position = position - self.center_position
        gradient = (4*LA.norm(relative_position)**2*relative_position
                      - 2*self.matrix.dot(relative_position))
        return gradient
    ...
    def test_numerical_gradient(self):
        ...
        for it_pos in range(positions.shape[1]):
            ...
            gradient_numerical = get_numerical_gradient(
                position=position,
                function=obstacle.get_function_value, delta_magnitude=1e-6)

            self.assertTrue(np.allclose(gradient_analytic, gradient_numerical, rtol=1e-5))
