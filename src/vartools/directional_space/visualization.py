...
def circular_space_setup(ax, circ_radius: float = pi/2, space_radius: float = pi,
                         circle_background_color='white', outer_boundary_color=None):
    ...
    if outer_boundary_color is not None:
        polygon = Point(0, 0).buffer(space_radius).difference(Point(0,0).buffer(circ_radius))
    ...
    path = pathify(polygon)
    patch = PathPatch(path, facecolor=circle_background_color, linewidth=1, linestyle='--')
    ax.add_patch(patch)
